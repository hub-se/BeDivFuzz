package de.hub.se.jqf.bedivfuzz.examples.chocopy;
...
public class SplitChocoPySemanticGenerator extends SplitGenerator<String> {
    ...
    private static List<String> generateItemsMultipleMethods(List<Function<SplitRandom, String>> genMethods, SplitRandom random, int minimum) {
        int len = nextStructureIntBound(random, minimum, maxBound, maxItems);
        List<String> items = new ArrayList<>(len);
        for (int i = 0; i < len; i++) {
    ...
    private static int nextStructureIntBound(SplitRandom random, int minimum, int maximum, int maxParam) {
        int randInt = random.nextStructureInt(minimum, maximum);
        assumeTrue(randInt <= maxParam);
        return randInt;
    }

    private static int nextValueIntBound(SplitRandom random, int minimum, int maximum, int maxParam) {
        int randInt = random.nextValueInt(minimum, maximum);
        assumeTrue(randInt <= maxParam);
        return randInt;
    }
