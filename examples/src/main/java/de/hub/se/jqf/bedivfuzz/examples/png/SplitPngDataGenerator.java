...
import edu.berkeley.cs.jqf.examples.png.ChunkBuilder;
import edu.berkeley.cs.jqf.examples.png.Filter;
...
public class SplitPngDataGenerator{
    // IHDR values
    ...
    public SplitPngDataGenerator(){}

    public byte[] generate(SplitRandom random) {
        ...
        try {
            ...
            if(tEXtUsed)
                for(int i = 0; i < random.nextStructureInt(1, 3); i++) {
    ...
    private void initializeParameters(SplitRandom random) {
        this.imageWidth = intToByteArray(random.nextValueInt(1, 100));
        this.imageHeight = intToByteArray(random.nextValueInt(1, 100));
        this.interlace = (byte) 0x00;

        initializeRandomColoring(random);

        this.tEXtUsed = random.nextStructureBoolean();
        this.zTXtUsed = random.nextStructureBoolean();
        this.iTXtUsed = random.nextStructureBoolean();
        this.gAMAUsed = random.nextStructureBoolean();
        this.cHRMUsed = random.nextStructureBoolean();
        this.sRGBUsed = random.nextStructureBoolean();
        if(sRGBUsed) {
            this.gAMAUsed = true;
            this.cHRMUsed = true;
        }
        this.bGKDUsed = random.nextStructureBoolean();
        this.pHYsUsed = random.nextStructureBoolean();
        this.sBITUsed = random.nextStructureBoolean();

        this.width = ByteBuffer.wrap(imageWidth).getInt();
        this.height = ByteBuffer.wrap(imageHeight).getInt();
    }

    private void initializeRandomColoring(SplitRandom random, int colorMethod, int bitDepth) {
        if (colorMethod == -1) {
            colorMethod = random.nextStructureInt(5);
        }

        switch (colorMethod) {
            case 0: // grayscale
                this.bitsPerChannel = (byte) ((int) Math.pow(2, random.nextValueInt(5)));
                this.colorType = 0x00;
                this.channels = 1;
            ...
            case 1: // grayscale with alpha
                this.bitsPerChannel = (byte) ((int) Math.pow(2, random.nextValueInt(3,4)));
                this.colorType = 0x04;
                this.channels = 2;
                this.backgroundMethod = 1;
                break;
            ...
            case 4: // indexed color, palette used
                this.bitsPerChannel = (byte) ((int) Math.pow(2, random.nextValueInt(4)));
                this.colorType = 0x03;
                this.channels = 1;
                this.PLTEUsed = true;
                if(random.nextStructureBoolean()) {
    ...
    private void initializeRandomColoring (SplitRandom random) {
        initializeRandomColoring(random, -1, -1);
    }

    private byte[] generateIHDR(SplitRandom random){
    ...
    private byte[] generateSBIT(SplitRandom random) {
        ByteArrayOutputStream sBit = new ByteArrayOutputStream();

        switch (colorType) {
            case 0:
                sBit.write(random.nextValueInt(1, bitsPerChannel));
                break;
            case 2:
                sBit.write(random.nextValueInt(1, bitsPerChannel));
                sBit.write(random.nextValueInt(1, bitsPerChannel));
                ...
                break;
            case 3:
                sBit.write(random.nextValueInt(1, 8));
                sBit.write(random.nextValueInt(1, 8));
                sBit.write(random.nextValueInt(1, 8));
                break;
            case 4:
    ...
    private byte[] generateCHRM(SplitRandom random) {
        ...
        try {
            if(sRGBUsed) { // sRGB uses fixed cHRM values
                ...
                for(int i = 0; i < 8; i++) {
                    cHRM.write(intToByteArray(random.nextValueInt(100001))); // randomized values
                }
            }
        }
        catch (IOException e) {
    ...
    private byte[] generateSRGB(SplitRandom random) {
        byte[] sRGB = new byte[]{(byte) random.nextValueInt(4)};
        return ChunkBuilder.constructChunk("sRGB".getBytes(), sRGB);
    }

    private byte[] generatePLTE(SplitRandom random) {
        ByteArrayOutputStream PLTE = new ByteArrayOutputStream();
        for(int i = 0; i < (int) Math.pow(2, bitsPerChannel); i++) {
            PLTE.write((byte) (random.nextValueInt((int) Math.pow(2, 8)))); // red
            PLTE.write((byte) (random.nextValueInt((int) Math.pow(2, 8)))); // green
            PLTE.write((byte) (random.nextValueInt((int) Math.pow(2, 8)))); // blue
        }
        return ChunkBuilder.constructChunk("PLTE".getBytes(), PLTE);
    }

    private byte[] generateTRNS(SplitRandom random) {
        ...
        try {
            switch (transparencyMethod) {
                case 0: // for indexed colors, artificial alpha palette
                    for (int i = 0; i < 256; i++) {
                        tRNS.write((byte) (random.nextValueInt((int) Math.pow(2, 8))));
                    }
                    break;
                case 1: // for grayscale, single alpha value for specified bytes
                    tRNS.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel))));
                    break;
                case 2: // for true color, single alpha value for specified bytes
                    tRNS.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // red
                    tRNS.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // green
                    tRNS.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // blue
                    break;
            }
        }
        catch (IOException e) {
    ...
    private byte[] generateBKGD(SplitRandom random) {
        ByteArrayOutputStream bKGD = new ByteArrayOutputStream();
        try {
            switch (backgroundMethod) {
                case 0: // for indexed colors
                    bKGD.write((byte) (random.nextValueInt((int) Math.pow(2, 8))));
                    break;
                case 1: // for grayscale
                    bKGD.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel))));
                    break;
                case 2: // for true color
                    bKGD.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // red
                    bKGD.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // green
                    bKGD.write(int2ToByteArray(random.nextValueInt((int) Math.pow(2, bitsPerChannel)))); // blue
                    break;
            }
        }
        catch (IOException e) {
    ...
    private byte[] generatePHYS(SplitRandom random) {
        ByteArrayOutputStream pHYs = new ByteArrayOutputStream();
        // specifies the aspect ratio between x and y in pixels
        try {
            pHYs.write(intToByteArray(random.nextValueInt(100))); // x
            pHYs.write(intToByteArray(random.nextValueInt(100))); // y
            pHYs.write((byte) random.nextValueInt(0, 1)); // unknown unit or meter
        }
        catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private byte[] generateTEXT(SplitRandom random) {
        ByteArrayOutputStream tEXt = new ByteArrayOutputStream();

        // Keyword
        for(int i = 0; i < random.nextStructureInt(1, 79); i++) {
            if(random.nextValueBoolean()) {
                tEXt.write((byte) random.nextValueInt(32, 126));
            } else {
                tEXt.write((byte) random.nextValueInt(161, 255));
            }
        }
        ...
        tEXt.write(0x00);
        // Text
        for(int i = 0; i < random.nextStructureInt(1, 256); i++) { // 256 (could be maximum chunk size - 80)
            int j = random.nextStructureInt(1, 100);
            if(j < 50) {
                tEXt.write((byte) random.nextValueInt(32, 126));
            } else if(j < 99) {
                tEXt.write((byte) random.nextValueInt(161, 255));
    ...
    private byte[] generateZTXT(SplitRandom random) {
        ...
        for(int i = 0; i < random.nextStructureInt(1, 79); i++) {
            if(random.nextValueBoolean()) {
                zTXt.write((byte) random.nextValueInt(32, 126));
            } else {
                zTXt.write((byte) random.nextValueInt(161, 255));
            }
        }
        ...
        for(int i = 0; i < random.nextStructureInt(1, 256); i++) { // 256 (could be maximum chunk size - 80)
            int j = random.nextStructureInt(1, 100);
            if(j < 50) {
                zTXt_text.write((byte) random.nextValueInt(32, 126));
            } else if(j < 99) {
                zTXt_text.write((byte) random.nextValueInt(161, 255));
            } else {
    ...
    private byte[] generateITXT(SplitRandom random) {
        ...
        boolean isCompressed = random.nextValueBoolean();
        ...
        try{
            ...
            if(random.nextStructureBoolean()) {
                iTXt.write(languages[random.nextValueInt(0, 3)].getBytes());
            }
            // Null separator
            iTXt.write(0x00);

            // Translated keyword
            iTXt.write(create_utf8(random, random.nextStructureInt(0, 100))); // this keyword can be longer than 79 bytes
            // Null separator
            iTXt.write(0x00);

            // Text
            iTXt_text.write(create_utf8(random, random.nextStructureInt(0, 256))); // 256 (could be maximum chunk size - other data bytes of this chunk)
        }
        catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private byte[] generateIDAT(SplitRandom random) {
        byte[] filteredData = generateFilteredData(random);
        ByteArrayOutputStream idat = new ByteArrayOutputStream();

        int compressionMethod = random.nextValueInt(-1,9);
    ...
    private byte[] generateFilteredData(SplitRandom random){
        ...
        for (int y = 0; y < height; y++) {
            ...
            int filterMethod = (byte) random.nextValueInt(5);
            ...
            for (int x = 1; x < scanline; x++) {
                ...
                byte imageByte = (byte) (random.nextValueInt((int) Math.pow(2, 8)));
    ...
    public byte[] create_utf8(SplitRandom random, int max_byte_number) {
        String str = "";
        for(int i = 0; i < max_byte_number - 1; i++) {
            str = str + random.nextValueChar((char) 0x0001, (char) 0x10FFFF); // uses maximum range of Unicode
        }
        byte[] bytes = Arrays.copyOfRange(str.getBytes(StandardCharsets.UTF_8), 0, str.length());
        if(bytes.length > 0 && bytes.length < 5) {
            for(int i = 0; i < bytes.length; i++) {
                bytes[i] = (byte) random.nextValueInt(1, 127);
            }
            return bytes;
        }
