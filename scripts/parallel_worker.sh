...
print_usage() {
  echo "Usage: $0 -o out_dir -t timeout -n repetitions -w worker_id [-r]"
}
...
while getopts ":o:t:n:w:r" opt; do
  case $opt in
    \?)
...
LOG_FILE=$([ -n "$COV_REPLAY" ] && echo "logs/coverage-experiments-$WORKER_ID.log" || echo "logs/crash-experiments-$WORKER_ID.log")
